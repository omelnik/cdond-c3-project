...
OLD_STACKS=$(aws cloudformation describe-stacks | jq -r '.Stacks[] | select(.StackName!="InitialStack").Parameters[].ParameterValue' | uniq)
...
do
    if [[ $stackId -ne $CURRENT_STACK_ID ]]; then
        echo "Deleting stack with the id: ${stackId}"
        S3_BUCKET="s3://udapeople-$stackId"
        if aws s3 ls $S3_BUCKET 2>&1 | grep -q 'An error occurred'; then
